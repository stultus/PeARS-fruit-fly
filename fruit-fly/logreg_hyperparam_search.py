...
def generate_projs(KC_size, proj_size):
    ...
    PN_size = len(vocab)
    d = "models/"+dataset_name+"/kc"+str(KC_size)+"-p"+str(proj_size)
...
def classify(tr_file,C,num_iter,config):
    ...
    with open('./log/'+dataset_name+'.results.tsv', 'a') as f:
        tmp = tr_file.split('.')
        kc = tmp[1][2:]
        size = tmp[2][4:]
        top = tmp[4][3:]
...
def fruitfly_pipeline(train_path, topword, KC_size, proj_size, percent_hash, C, num_iter):
    ...
    config = "kc"+str(KC_size)+"-p"+str(proj_size)+"-t"+str(topword)+"-h"+str(percent_hash)+"-C"+str(C)+"-i"+str(num_iter)
    hash_path = "tmp/"+dataset_name+"/"+config
    os.mkdir(hash_path)
    print('hashing files')
    hash_file = hash(model_file=model_file, in_file_path=train_path,
                     output_dir=hash_path, top_tokens=topword, percent_hash=percent_hash)
